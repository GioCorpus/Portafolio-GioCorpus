  ...
  return (
    <article className="group relative pl-12">
      <div className="absolute left-0 top-0 w-6 h-6 flex items-center justify-center">
        <div className={`w-2 h-2 rounded-full bg-[var(--item-color)]/50 ring-2 ring-dark-950 transition-all group-hover:scale-150 group-hover:ring-[var(--item-color)]/50`} style={{ '--item-color': `var(--color-${color})` }} aria-hidden="true" />
        <div className={`absolute w-1.5 h-full bottom-full bg-[var(--item-color)]/20`} style={{ '--item-color': `var(--color-${color})` }} aria-hidden="true" />
      </div>

      <div className="glass-panel rounded-xl p-6 border border-white/5 transition-all duration-300 hover:border-accent-cyan/30 hover:bg-white/10">
        <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
          <div className="flex-1 min-w-[200px]">
            <div className="flex items-center gap-2 mb-2">
              <Icon className={`w-4 h-4 text-[var(--item-color)]`} style={{ '--item-color': `var(--color-${color})` }} aria-hidden="true" />
              <span className="font-mono text-xs uppercase tracking-wider text-[var(--item-color)]/70" style={{ '--item-color': `var(--color-${color})` }}>{label}</span>
            </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {exp.technologies.length > 6 && (
            <span className="px-2 py-1 text-[10px] font-mono bg-white/5 border border-white/10 rounded text-dark-500">
          ...
          <ul className="mt-4 space-y-2">
            ...
            {exp.achievements.length > 3 && (
              <li className="flex gap-2 text-sm text-dark-500">
                ...
                <span className="line-clamp-1">+{exp.achievements.length - 3} more achievements</span>
              </li>
            )}
          </ul>
        )}
...
export function ExperienceSnapshot() {
  ...
  return (
    <section id="experience" className="border-y border-white/5 bg-white/[0.015] px-4 sm:px-6 lg:px-8 py-20 sm:py-28">
      <div className="mx-auto max-w-7xl">
        <div className="mb-12 max-w-3xl">
          <p className="mb-3 font-mono text-xs uppercase tracking-[0.3em] text-accent-cyan">04 / Experience</p>
